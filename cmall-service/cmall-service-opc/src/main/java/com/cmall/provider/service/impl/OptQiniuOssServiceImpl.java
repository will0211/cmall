...
public class OptQiniuOssServiceImpl implements OpcOssService {
	...
	private static final String OPEN_IMG_BUCKET = "data2-public";
	...
	public OptUploadFileRespDto uploadFile(byte[] uploadBytes, String fileName, String filePath, String bucketName) throws IOException {
		...
		log.info("uploadFile token:{}",token);
		ByteArrayInputStream byteInputStream=new ByteArrayInputStream(uploadBytes);
		uploadManager.put(byteInputStream,fileName,token,null, null);
		Response response = uploadManager.put(uploadBytes, filePath + newFileName, token);
		...
		if (StringUtils.equals(OPEN_IMG_BUCKET, bucketName)) {
			fileUrl = cmallProperties.getQiniu().getOss().getPublicHost() + "/" + filePath + newFileName;
		} else {
			String domainUrl = cmallProperties.getQiniu().getOss().getPrivateHost();
	...
	private void checkFileSize(byte[] uploadFileByte) {
		long redisFileSize;
		Long fileMaxSize = cmallProperties.getQiniu().getOss().getFileMaxSize();
